...
func readFileFromProviderResourceDir(t *testing.T, filename string) []byte {
	...
	b, err := os.ReadFile(filepath.Join("..", "..", "cmd", "pulumi-resource-forgejo", filename))
	if err != nil {
...
func makeTestProvider(ctx context.Context, t *testing.T) pulumirpc.ResourceProviderServer {
	...
	_, err = p.Configure(ctx, &pulumirpc.ConfigureRequest{
		Variables: map[string]string{"forgejo:config:apiKey": "fakeapikey"},
	})
...
func TestDiff(t *testing.T) {
	...
	resp, err := p.Diff(ctx, &pulumirpc.DiffRequest{Id: "", Urn: "urn:pulumi:some-stack::some-project::forgejo:services:StaticSite::someResourceName", Olds: oldsStruct, News: newsStruct})
	assert.Nil(t, err)
...
func TestCreate(t *testing.T) {
	...
	_, err := p.Create(ctx, &pulumirpc.CreateRequest{
		Urn:        "urn:pulumi:dev::forgejo-ts::forgejo:services:WebService::webservice",
