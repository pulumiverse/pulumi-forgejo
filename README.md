...
This boilerplate creates a working Pulumi-owned provider named `forgejo`.
...
1. Set the following options:
   ...
   - Repository name: pulumi-forgejo (replace "forgejo" with the name of your provider)
   ...
   - Description: Pulumi provider for forgejo
...
| Search | Replace With                                                                   |
| ------ | ------------------------------------------------------------------------------ |
| forgejo    | Lower-cased name of the provider                                               |
| Forgejo    | Pascal-case name of the provider                                               |
| FORGEJO    | Upper-cased name of the provider                                               |
| forgejo  | Lower snake-cased name of the provider if the provider name has multiple words |
...
1. A `provider/` folder containing the building and implementation logic
   1. `cmd/`
      1. `pulumi-gen-forgejo/` - generates language SDKs from the schema
      2. `pulumi-resource-forgejo/` - holds the package schema, injects the package version, and starts the gRPC server
   2. `pkg`
...
3. `sdk` - holds the generated code libraries created by `pulumi-gen-forgejo/main.go`
